...
def gettittle():
    ...
    tittle_list = json.loads(res)["list"]
    res = tittle_list[0]
    logging.info(res)
    id = res["id"]
    tittle = res["title"]
    ...
    logging.info("第{}期".format(id) + "青年大学习标题：" + tittle + "\n" + "青年大学习学习链接：" + url)
    return id, tittle, url,tittle_list
...
def get_score_viery_history(token,userid):
    url = f"http://www.jxqingtuan.cn/pub/pub/vol/classrecord/index?openid={token}&userId={userid}&page=1&pageSize=10"
    response = requests.request("GET", url, headers=config.headers,timeout=5)
    print(json.loads(response.text)['vo'])


def historystudy(token,nid,name,id,tittle,subOrg,url,tittle_list):
    global error
    print(f'{tittle}\nNID: {nid}\nName: {name}\n{subOrg}\n')
    try:
        QN_study(token, token, id, nid, name, subOrg)
        ...
        for i in tittle_list:
            ...
            addScoreInfo(token, userid, url)
            print(f'{title} 学习完成\n')
        get_score_viery_history(token, userid)
    except Exception as e:
        logging.error("main 函数出现错误，错误为：", e)
        info_list = {}
        info_list['nid'] = nid
    ...
    if agree == 0:
        ...
        while 1:
            ...
            if mod == 1 :
                ...
                data = get_excel_info()
                id, tittle, url,tittle_list = gettittle()
                for i in tqdm(data):
                    token = i['token']
                    nid = i['nid']
                    name = i['name']
